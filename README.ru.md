...
Версия `1.3.1` отделяет сырой журнал hooks от полезной памяти: журнал доступен для явного просмотра, но не попадает в поиск, автоматический контекст и семантический индекс. Обработчик может пропускать рутинные действия; короткие ссылки на источники ограничены схемой ответа. Нативная интеграция проверена на Codex CLI `0.153.4`; эта версия задаёт границу подтверждённой совместимости.

Для подходящих событий инструмента сохраняется до 2 000 символов результата после удаления приватных блоков и распознаваемых секретов; общий предел события — 6 000 символов. Полные транскрипты и изображения не копируются.
...
Хуки собирают наблюдения по ходу работы. `memory` помогает использовать эти сведения и сохранять важные выводы. `maintenance` обслуживает систему сбора, обработки и поиска.
