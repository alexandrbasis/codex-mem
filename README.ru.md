...
Версия `1.9.0` показывает свежесть памяти в поиске и контексте сессии: время захвата и обработки, ожидающие события, блокировки и отдельное покрытие индекса. Добавлены точечное восстановление проверенных runner/storage-сбоев без повторов контентного карантина и ускоренный выбор событий для обработки.

Версия `1.8.0` добавляет отчёты за выбранный период с отдельными оценками стоимости по тарифам API и расхода кредитов Codex. По умолчанию отчёт читает локальную базу; явное обновление импортирует ограниченную порцию метаданных без вызова модели. Сборщик восстанавливает пропущенный режим Standard/Fast и различает запрошенные настройки и подтверждённые провайдером. Обработчик сохраняет расход по ходу попытки, чтобы восстановление после сбоя могло сохранить частичный итог. Обработчик использует Luna с reasoning `medium`. Сырой журнал доступен для явного просмотра и исключён из обычного поиска, автоматического контекста и семантического индекса. Граница нативной совместимости: Codex CLI `0.153.4`. Результаты проверок и установки приведены в [VERIFICATION.md](docs/VERIFICATION.md).
...
Посмотреть расход и оценку стоимости за 11 и 12 сентября по времени Израиля:
...
Граница `--from` включается в период, `--to` не включается. Даты интерпретируются в выбранном часовом поясе IANA; точное время должно содержать смещение относительно UTC. Без дат период начинается вчера в полночь и заканчивается текущим моментом. Часовой пояс по умолчанию: `UTC`.

Без `--project` отчёт охватывает все сохранённые локальные проекты. Добавьте `--project /absolute/path/to/project` для выбора проекта и `--session-id ROOT_SESSION_ID` для выбора корневой задачи и её агентов. Вместо `ROOT_SESSION_ID` укажите ID сессии задачи. Флаг `--group-by` принимает значения `day,project,task,agent,model` через запятую; по умолчанию возвращаются все пять разбивок.

Каждая разбивка возвращается отдельно в `groups.main` и `groups.observer`. В ней не более 100 групп и указано число пропущенных; итоговые суммы включают все подходящие записи.

В MCP большие разбивки могут быть дополнительно сокращены из-за ограничения размера ответа. Это отмечается в `transport.truncated` и числе пропущенных групп; итоговые суммы сохраняются. Для подробностей сузьте проект, период или набор разбивок.

По умолчанию отчёт читает базу без импорта журналов и вызова модели. Чтобы сначала импортировать ограниченную порцию данных, добавьте `--refresh`. Обновление можно запустить отдельно:
...
Обновление соблюдает настройки сбора и изменяет только метаданные расхода. Оно может сохранить события за пределами выбранного периода из обрабатываемого файла: для определения их контекста нужно читать файл последовательно. По умолчанию за проход обрабатываются до 32 файлов и 8 МиБ. Флаг `--max-files` принимает 1–128, `--max-bytes` принимает 1–8388608. Повторите обновление, если покрытие остаётся неполным.

В MCP тот же отчёт доступен через инструмент только для чтения `memory_usage_report`:
...
Для импорта используйте `memory_usage_refresh` с периодом и необязательными `project`, `session_id`, `max_files`, `max_bytes`, затем снова вызовите `memory_usage_report`. Инструмент отчёта сам не обновляет журнал. Оба инструмента работают без вызова модели.

### Стоимость и полнота расчёта

Общий калькулятор сначала рассчитывает каждый ответ, затем складывает суммы. Он учитывает модель, режим Standard/Fast, кэшированный вход, запись кэша и тариф для длинного контекста из снимка `openai-2026-09-12.1`. Эквивалент в долларах по [тарифам OpenAI API](https://developers.openai.com/api/docs/pricing) и оценка по [тарифам кредитов Codex](https://learn.chatgpt.com/docs/pricing#token-rates) считаются отдельно. Применение этого снимка к старому расходу не восстанавливает исторический счёт. Фактические платежи за подписку, покупки кредитов, налоги, сборы за инструменты и скидки аккаунта недоступны.

У каждого показателя стоимости `selected_subtotal` содержит сумму по записям с известной оценкой. Если хотя бы одну включённую запись оценить нельзя, `total` равен `null`. Если тариф известен, а режим нет, сценарии Standard и Fast показывают условные суммы. Запрошенные настройки отмечаются отдельно от подтверждённых провайдером. Неизвестный тариф или режим не превращается в бесплатную запись. Кэшированный вход уже входит во входные токены, reasoning уже входит в выходные. Старые накопительные записи не содержат точных границ ответов, поэтому оценка по размеру контекста для них приблизительна.

Смотрите на покрытие вместе с суммой. Оно показывает пропуски в определении модели и режима, известные недочитанные файлы, необходимость повторного разбора, ограничения проверки и ошибки. Покрытие файлов относится ко всей базе даже в отчёте по проекту: проект непрочитанного файла ещё неизвестен. Отчёт по сохранённой базе не обнаруживает новые файлы. Ограниченный или незавершённый поиск файлов не подтверждает полный учёт.

Для частичных или ещё работающих фоновых попыток полная стоимость `total` тоже остаётся неизвестной; сохранённая часть расхода входит в промежуточные суммы. Публичная таблица кредитов Codex не задаёт цену записи кэша, поэтому для таких записей оценка в кредитах недоступна, даже если стоимость по API можно рассчитать.

После обновления следующий проход сбора или `usage refresh` повторно разбирает старые файлы для восстановления вложенных настроек Standard/Fast. Постоянные ID ответов позволяют дополнить существующие записи без повторного начисления токенов. Отсутствующие исходные файлы и расход, который никогда не записывался, остаются неизвестными.

### Учёт фоновой обработки

Отчёт за период отдельно показывает основные сессии, попытки фонового обработчика и общую сумму. Он исключает сессии обработчика из основного потока по ID, чтобы не считать их дважды. Весь расход попытки относится к дате её начала, даже если она пересекает полночь или границу выбранного периода. Исторические попытки без записей расхода показаны как пробел за всё время в выбранной области; их стоимость и точную дату определить нельзя.

Посмотреть прежние сводки токенов проекта:
...
Обработчик сохраняет последний корректный итог расхода от app-server для своей новой сессии и хода, в том числе промежуточные снимки во время работы. Повторные сообщения заменяют снимок, а не суммируются. При восстановлении просроченной попытки уже сохранённый частичный расход остаётся в журнале. У каждой повторной попытки есть отдельная запись, поэтому её сообщённый расход учитывается отдельно. Также сохраняются результат попытки и доступная длительность.
...
Учёт описывает сохранённые попытки. Оценка стоимости не подтверждает фактические списания или общую экономию от памяти. Границы проверок приведены в [VERIFICATION.md](docs/VERIFICATION.md), источники протокола и сравнения в [UPSTREAM.md](UPSTREAM.md).
