...
Версия `1.6.0` добавляет компактные результаты поиска, историю вокруг выбранной записи, исправления очереди, приоритет свежих данных расхода и проверку обновления работающего сервиса. Обработчик использует Luna с reasoning `medium`. Сырой журнал доступен для явного просмотра и исключён из обычного поиска, автоматического контекста и семантического индекса. Граница нативной совместимости — Codex CLI `0.153.4`.
...
Диагностика показывает доступные данные расхода токенов и возраст последнего события. Недоступные данные не считаются нулевым расходом. Если sandbox запрещает проверку процесса, его состояние обозначается как `unknown`. Расход самого обработчика в ephemeral-сессиях пока не охвачен подтверждённым учётом; общую экономию системы по этому журналу посчитать нельзя.
