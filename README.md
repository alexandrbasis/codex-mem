...
Search also reports project-scoped `freshness`: latest capture, last successful processing, last ready record, pending captures, blocker, and separate semantic index counts. A full index of ready notes can coexist with an unprocessed capture backlog. `knowledge_incomplete` is `true` for a known backlog or blocker, otherwise `null` when disabled or unknown, and `false` only when no retained local backlog is known. Capture, processing, and semantic enablement are reported separately. An idle project is not stale merely because its timestamps are old. The snapshot does not verify external events or native hook execution.

MCP preserves compact result lists and includes freshness in its additional retrieval text block and `_meta.codexMemRetrieval`. CLI search preserves the legacy stdout list and emits freshness JSON on stderr; supplying `--mode` or `--intent` returns a JSON envelope with `results` and `freshness`. Empty searches still include the diagnostic. Automatic SessionStart context and the `context` command put a freshness notice before notes within the existing character budget; tiny budgets explicitly omit detailed metrics. These reads do not start a worker, retry a job, or modify memory, configuration, or queue state.
